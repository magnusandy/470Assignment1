...
#define DEFAULT_ITERATION_COUNT (1000)
...
int (*runScenario(int countSteps, bool (*pFnComputeLiveness)(bool isCurrentCellAlive, int countSurroundingAlive)))[COL_COUNT];
int (*updateSpace(int iTime, bool (*pFnComputeLiveness)(bool isCurrentCellAlive, int countSurroundingAlive)))[COL_COUNT];
void updateCell(int (*currentCells)[COL_COUNT], int (*nextCells)[COL_COUNT], int row, int col, int (*pFnComputeLiveness)(bool isCurrentCellAlive, int countSurroundingAlive));
...
int countSurroundingLiveCells(int (*currentCells)[COL_COUNT], int row, int col);
int indexForRowCol(int row, int col);
void determineCurrentAndNextCells(int iTime, int (**pArrayCurrentCells)[COL_COUNT], int (**pArrayNextCells)[COL_COUNT]);
bool isLegalCoord(int row, int col);
void readState(char *strFileName, int (*arrayCurrentCells)[COL_COUNT]);
void  writeState(char *strFileName, int (*arrayCurrentCells)[COL_COUNT]);
...
typedef bool (*livenessRuleType)(bool isCurrentCellAlive, int countSurroundingAlive);
...
int main(int argc, char *argv[])
{
        bool (*pFnComputeLiveness)(bool isCurrentCellAlive, int countSurroundingAlive);
        int countIterations;

        if (!determineArgumentsReturnIsLegal(argc, argv, &countIterations, &pFnComputeLiveness))
        {
                fprintf(stderr, "Syntax:  'conway [iterations] [rule]', where iterations is a value integer >0.  If no iteration count is given, %d iterations are assumed, and 'rule' is one of 'Classic', 'Liveness2To4', 'Liveness2To3'.\n", DEFAULT_ITERATION_COUNT);
                exit(1);
        }
        else
        {
                readState("input.txt", bufferEven);
                int (*finalState)[COL_COUNT] = runScenario(countIterations, pFnComputeLiveness);
                writeState("output.txt", bufferEven);

                exit(0);
        }
}

// Run Conway's Game of Life or variant (as specified by cell mapping function pFnComputeLiveness) for countSteps steps, on the initial state given by bufferEven, and returns a pointer to a 2d array encoding the final state.
// PRECONDITIONS:
//    countSteps >= 0
//		bufferEven contains the initial state of the model (as a 2d array of rows/columns)
...
// Run a single iteration of Conway's Game of Life or variant (as specified by cell mapping function pFnComputeLiveness) on the current space at time iTime (as determined by calling determineCurrentAndNextCells for iTime), and returns a pointer to an array encoding the final state (in 2d array form)
// PRECONDITIONS:
//    countSteps >= 0
//		bufferEven contains the initial state of the model (as a 2d array of rows/columns),
...
//		pFnComputeLiveness is a pointer to a function that -- given an indication if the current cell is alive, and the count of live surrounding neighbors -- returns true if the cell should stay alive, false otherwise
// POSTCONDITIONS:
//		returns a pointer to the final state of the model, encoded in the same way as was bufferEven.  This final state was produced by
//		running Conway's Game of Life for countSteps, starting with the initial state of the model
int (*updateSpace(int iTime, bool (*pFnComputeLiveness)(bool isCurrentCellAlive, int countSurroundingAlive)))[COL_COUNT]
{
  //fprintf(stderr, "%s\n", "");
        int (*currentCells)[COL_COUNT];
        int (*nextCells)[COL_COUNT];
...
// For the cell at row row and column col in state given by currentCells, perform the rule specified by cell mapping function pFnComputeLiveness  and place the results in nextCells. Both cells are in 2d array form).
// PRECONDITIONS:
...
//		currentCells contains a contains a legal encoding ((as a 2d array of rows/columns)) of the matrix constituting the current state of the model of dimension ROW_COUNT x COL_COUNT, where the content of each patch is given by an integer,
...
//
void updateCell(int (*currentCells)[COL_COUNT], int (*nextCells)[COL_COUNT], int row, int col, bool (*pFnComputeLiveness)(bool isCurrentCellAlive, int countSurroundingAlive))
{
        int countSurroundingAlive = countSurroundingLiveCells(currentCells, row, col);
        bool isCurrentCellAlive = currentCells[row][col];
      //  fprintf(stderr, "%i", countSurroundingAlive);
...
// Given the current cell at row row and column col in the state specified by array currentCells (as 2d array, with each patch
// encoded by 1 if live and 0 if empty), indicate the total count of the cells that are occupied in the surrounding WRAPPED Moore Neighbourhood (i.e., in the 8 surrounding cells in the North, NorthEast, East, South East, South, South West, West, and North West directions) and cells on the edge will check the opposite edge as its neighbours
// PRECONDITIONS:
//		currentCells contains a contains a legal encoding ((as a 2d array of rows/columns)) of the matrix constituting the current state of the model of dimension ROW_COUNT x COL_COUNT, where the content of each patch is given by an integer,
//			with an empty patch denoted by 0, and an occupied (i.e., live) patch indicated by 1.
...
// POSTCONDITIONS:
//		returns the total count of the cells that are occupied in the surrounding Moore Neighbourhood (i.e., in the 8 surrounding cells in the North, NorthEast, East, South East, South, South West, West, and North West directions in a wrapping fashion)
int countSurroundingLiveCells(int (*currentCells)[COL_COUNT], int row, int col)
{
        ...
        for (int deltaRow = -1; deltaRow <= 1; deltaRow++)
                for (int deltaCol = -1; deltaCol <= 1; deltaCol++)
                {
                        ...
                        //adding functionality for wraping space, if the row to be checked goes over the end of the space
                        //go to the other side of the space, i.e. the neighbour one left of the first column is the last column
                        if(checkingRow == -1) checkingRow=ROW_COUNT-1;
                        if(checkingRow == ROW_COUNT) checkingRow=0;
                        if(checkingCol == -1) checkingCol=COL_COUNT-1;
                        if(checkingCol == COL_COUNT) checkingCol=0;
...
//    0 <= time
// POSTCONDITIONS:
//		*pArrayCurrentCells is set to hold a pointer to the current array at timestep iTime (i.e., the 2d array that holds the value of the current state at that time)
//		*pArrayNextCells is set to hold a pointer to the next array at at timestep iTime (i.e., to the 2d array that will hold the values for the next iteration)
void determineCurrentAndNextCells(int iTime, int (**pArrayCurrentCells)[COL_COUNT], int (**pArrayNextCells)[COL_COUNT])
{
...
//		arrayCurrentCells points to an 2d array of dimension ROW_COUNT x COL_COUNT
// POSTCONDITIONS:
//		The contents of file strFileName are placed in arrayCurrentCells in 2d array form
void readState(char *strFileName, int (*arrayCurrentCells)[COL_COUNT])
{
...
//		arrayCurrentCells contains a contains a legal encoding (as a 2d array of rows/columns) of the matrix constituting the current state of the model of dimension ROW_COUNT x COL_COUNT, where the content of each patch is given by an integer,
//			with an empty patch denoted by 0, and an occupied (i.e., live) patch indicated by 1.
// POSTCONDITIONS:
...
bool determineArgumentsReturnIsLegal(int argc, char *argv[], int *pCountIterations, livenessRuleType *ppFnComputeLiveness)
{
        if (argc == 1)
        {
                ...
                *pCountIterations = DEFAULT_ITERATION_COUNT;
                *ppFnComputeLiveness = computeLivenessClassic;
                return(true);
        }
        else
        {
                ...
                if (countIterations > 0)
                        *pCountIterations = countIterations;
                else
                {
                        *pCountIterations = -1; // indicate an illeal value
                        return(false);
                }
                ...
                else if (argc == 3)
                {
                        ...
                        *ppFnComputeLiveness = determineRuleFunctionForCommandLineArgument(argv[2]);

                        return (*ppFnComputeLiveness != NULL); // if the rule is null, indicate false, otherwise true
                }
                else
                        return false; // signal that too many arguments are given
        }
}
...
{
        char *strArgument;
        livenessRuleType pFnRule;
} commandLineDispatchEntry;


#define COUNT_COMMAND_LINE_RULE_ENTRIES 3
commandLineDispatchEntry commandLineDispatchTable[COUNT_COMMAND_LINE_RULE_ENTRIES] =
{
        { "Classic", computeLivenessClassic },
        { "Liveness2To4", computeLivenessRemainAlive2Thru4 },
        { "Liveness1To3", computeLivenessRemainAlive1Thru3 }
};


livenessRuleType determineRuleFunctionForCommandLineArgument(char *strCommandLineArg)
{
        for (int i = 0; i < COUNT_COMMAND_LINE_RULE_ENTRIES; i++)
        {
                if (strcmp(commandLineDispatchTable[i].strArgument, "Classic") == 0)
                        return(commandLineDispatchTable[i].pFnRule);
        }
